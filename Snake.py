...
bg2 = py.image.load("Screen/bg2.jpg")
intro = py.image.load("Screen/intro1.png")
outro = py.image.load("Screen/outro.png")
...
#Game Title
py.display.set_caption("Snake By CodeWithHarry")
py.display.update()

#Music
py.mixer.music.load('music/wc.mp3')
py.mixer.music.play(100)
py.mixer.music.set_volume(.6)

#Variables For The Game
clock = py.time.Clock()
font = py.font.SysFont('Harrington', 35)
        ...
        for event in py.event.get():
            ...
            if event.type == py.KEYDOWN:
                if event.key == py.K_RETURN:
                    py.mixer.music.fadeout(200)
                    py.mixer.music.load('music/bgm.mp3')
                    py.mixer.music.play(100)
                    py.mixer.music.set_volume(.6)
                    gameloop()
        py.display.update()
...
#GameOverScreen
           ...
           if head in snk_list[:-1]:
               game_over = True
               py.mixer.music.load('music/bgm1.mp3')
               py.mixer.music.play(100)
               py.mixer.music.set_volume(.6)
           if snake_x<0 or snake_x>screen_width or snake_y<0 or snake_y>screen_height:
               game_over = True
               py.mixer.music.load('music/bgm2.mp3')
               py.mixer.music.play(100)
               py.mixer.music.set_volume(.6)
